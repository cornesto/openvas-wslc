# ---------------------------------------------------------
# OpenVAS Community Edition PowerShell Module
# ---------------------------------------------------------
...
Function Test-OpenVASInstalled {
    ...
    wslc inspect $Containers[0] 2>$null | Out-Null
    return ($LASTEXITCODE -eq 0)
}

Function Show-MenuHeader {
    Clear-Host
    Write-Host "  ___  ____  _____ _   _ __     ___    ____ " -ForegroundColor Cyan
    Write-Host " / _ \|  _ \| ____| \ | |\ \   / / \  / ___|" -ForegroundColor Cyan
    Write-Host "| | | | |_) |  _| |  \| | \ \ / / _ \ \___ \" -ForegroundColor Cyan
    Write-Host "| |_| |  __/| |___| |\  |  \ V / ___ \ ___) |" -ForegroundColor Cyan
    Write-Host " \___/|_|   |_____|_| \_|   \_/_/   \_\____/" -ForegroundColor Cyan
    Write-Host "       M A N A G E M E N T   T O O L        `n" -ForegroundColor DarkCyan
    
    Write-Host "===============================================" -ForegroundColor White
    Write-Host " OpenVAS Management Tool v$ModuleVersion (Windows x64/ARM64) " -ForegroundColor White
    Write-Host "===============================================" -ForegroundColor White
    Write-Host " Community Edition (immauss/openvas) " -ForegroundColor DarkGray
    Write-Host "===============================================" -ForegroundColor White
...
Function Show-SystemStatus {
    ...
    if (-not $IsInstalled) {
        ...
        $rawStatus = (wslc inspect $Containers[0] 2>$null | ConvertFrom-Json)
        ...
        Write-Host "$($Containers[0]) is $status" -ForegroundColor $statusColor
        
        $uiStatus = "Offline - Use Option L to view logs"
        $uiColor = "Red"
        if ($status -eq "running") {
            try {
                $null = Invoke-WebRequest -Uri "http://127.0.0.1:9392" -UseBasicParsing -TimeoutSec 5 -ErrorAction Stop
...
Function Deploy-OpenVASContainers {
    ...
    Write-Progress -Activity $Activity -Status "Starting $($Containers[0]) (1/1)" -PercentComplete 100

    wslc run -d --name $Containers[0] $PlatformFlag `
        -p 9392:9392 `
        -p 8080:80 `
        -e SKIPSYNC=true `
        -v openvas_immauss_data:/data `
        $Images[0] 2>&1 | Out-Null
...
Function Set-OpenVASLimits {
    if (-not (Confirm-OpenVASInstalled)) { return }
    ...
    if ($status -ne "running") {
        ...
        wslc start $Containers[0] 2>$null | Out-Null
        $needsStop = $true
        Start-Sleep -Seconds 5
    }

    Write-Host "Configuring OpenVAS limits inside the container (/etc/openvas/openvas.conf)..." -ForegroundColor Yellow
    wslc exec $Containers[0] /usr/bin/sed -i '/max_hosts/d' /etc/openvas/openvas.conf 2>$null
    wslc exec $Containers[0] /usr/bin/sed -i '/max_checks/d' /etc/openvas/openvas.conf 2>$null
    wslc exec $Containers[0] /bin/sh -c "echo 'max_hosts = 4' >> /etc/openvas/openvas.conf" 2>$null
    wslc exec $Containers[0] /bin/sh -c "echo 'max_checks = 4' >> /etc/openvas/openvas.conf" 2>$null
    ...
    wslc exec $Containers[0] cat /etc/openvas/openvas.conf 2>$null

    if ($needsStop) {
        Write-Host "Stopping container..." -ForegroundColor Yellow
        wslc stop $Containers[0] 2>$null | Out-Null
    } else {
...
Function Restart-OpenVAS {
    ...
    if ($status -eq "running") {
        Write-Host "Stopping $($Containers[0])..."
        wslc stop $Containers[0] 2>$null | Out-Null
    }
    
    Write-Host "Starting $($Containers[0])..."
    wslc start $Containers[0] 2>$null | Out-Null
    ...
    while ($stopwatch.Elapsed.TotalSeconds -lt $timeout) {
        try {
            $null = Invoke-WebRequest -Uri "http://127.0.0.1:9392" -UseBasicParsing -TimeoutSec 2 -ErrorAction Stop
            $isOnline = $true
            break
        } catch {
            if ($_.Exception.Response) {
...
Function Confirm-OpenVASInstalled {
    if (Test-OpenVASInstalled) {
        return $true
    }
...
Function Update-OpenVASImages {
    ...
    Write-Progress -Activity $Activity -Status "Downloading $($Images[0])" -PercentComplete 100
    $pullOutput = wslc pull $Images[0] 2>&1
    ...
    if (Test-OpenVASInstalled) {
        ...
        wslc rm -f $Containers[0] 2>$null | Out-Null
        Deploy-OpenVASContainers
    } else {
...
Function Uninstall-OpenVAS {
    ...
    if (Test-OpenVASInstalled) {
        ...
        Write-Progress -Activity $Activity -Status "Removing container: $($Containers[0])" -PercentComplete 50
        wslc rm -f $Containers[0] 2>$null | Out-Null 
...
Function Show-OpenVASLogs {
    ...
    wslc logs --tail 50 $Containers[0]
}
...
Function Sync-OpenVASFeeds {
    ...
    if ($null -ne $rawStatus -and $rawStatus[0].State.Status -eq "running") {
        ...
        wslc exec $Containers[0] /bin/sh -c "greenbone-feed-sync --type all"
...
Function Invoke-OpenVAS {
    ...
    while ($menuLoop) {
        ...
        switch -Regex ($Choice) {
            ...
            '^[Ee]$' { 
                Write-Host "Exiting OpenVAS Management Tool." -ForegroundColor Cyan
                $menuLoop = $false 
            }
            Default { 
...
Function Manage-OpenVAS {
    ...
    #>
    [CmdletBinding()]
    [Diagnostics.CodeAnalysis.SuppressMessageAttribute('PSUseApprovedVerbs', 'Manage-OpenVAS')]
    param()

    Invoke-OpenVAS
}

Set-Alias -Name Start-OpenVAS -Value Invoke-OpenVAS

Export-ModuleMember -Function Invoke-OpenVAS, Manage-OpenVAS -Alias Start-OpenVAS
